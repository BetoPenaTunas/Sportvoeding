...
public class ControladorTienda extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        String accionServer=request.getParameter("accionCliente");
        
        if(accionServer.equals("muestraProductos")){
    ...
    private void muestraProductos(HttpServletRequest request, HttpServletResponse response) {
        try {
            List listadoProductosValue;
            ModeloProducto mp = new ModeloProducto();
            listadoProductosValue=mp.getAllProductos();
            request.setAttribute("listadoProductosVar", listadoProductosValue);
            request.getRequestDispatcher("Tienda2.jsp").forward(request, response);
